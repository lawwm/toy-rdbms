...
struct Tuple {
  ...
  bool operator==(Tuple& other) {
    if (this->fields.size() == other.fields.size()) {
      for (int k = 0; k < other.fields.size(); ++k) {
        if (this->fields[k]->getConstant() != other.fields[k]->getConstant()) {
          return false;
        }
      }
    }
  ...
  friend std::ostream& operator<<(std::ostream& ostrm, const Tuple& tuple);
};
